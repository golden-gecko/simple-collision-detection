...
namespace Collision
{
	...
	bool Solver::collide(const OBB& s1, const OBB& s2)
	{
		...
		Collision::Vector3 t = b.getPosition() - a.getPosition();

		// Bring translation into a's coordinate frame
		t = Collision::Vector3(t.dotProduct(a.u[0]), t.dotProduct(a.u[2]), t.dotProduct(a.u[2]));
		...
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 3; j++)
			{
				AbsR[i][j] = Collision::Math::Abs(R[i][j]) + EPSILON;
			}
		}
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (Collision::Math::Abs(t[i]) > ra + rb)
			{
				return false;
			}
		}
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (Collision::Math::Abs(t[0] * R[0][i] + t[1] * R[1][i] + t[2] * R[2][i]) > ra + rb)
			{
				return false;
			}
		}
		...
		if (Collision::Math::Abs(t[2] * R[1][0] - t[1] * R[2][0]) > ra + rb) return false;

		// Test axis L = A0 x B1
		ra = a.e[1] * AbsR[2][1] + a.e[2] * AbsR[1][1];
		rb = b.e[0] * AbsR[0][2] + b.e[2] * AbsR[0][0];
		if (Collision::Math::Abs(t[2] * R[1][1] - t[1] * R[2][1]) > ra + rb) return false;

		// Test axis L = A0 x B2
		ra = a.e[1] * AbsR[2][2] + a.e[2] * AbsR[1][2];
		rb = b.e[0] * AbsR[0][1] + b.e[1] * AbsR[0][0];
		if (Collision::Math::Abs(t[2] * R[1][2] - t[1] * R[2][2]) > ra + rb) return false;

		// Test axis L = A1 x B0
		ra = a.e[0] * AbsR[2][0] + a.e[2] * AbsR[0][0];
		rb = b.e[1] * AbsR[1][2] + b.e[2] * AbsR[1][1];
		if (Collision::Math::Abs(t[0] * R[2][0] - t[2] * R[0][0]) > ra + rb) return false;

		// Test axis L = A1 x B1
		ra = a.e[0] * AbsR[2][1] + a.e[2] * AbsR[0][1];
		rb = b.e[0] * AbsR[1][2] + b.e[2] * AbsR[1][0];
		if (Collision::Math::Abs(t[0] * R[2][1] - t[2] * R[0][1]) > ra + rb) return false;

		// Test axis L = A1 x B2
		ra = a.e[0] * AbsR[2][2] + a.e[2] * AbsR[0][2];
		rb = b.e[0] * AbsR[1][1] + b.e[1] * AbsR[1][0];
		if (Collision::Math::Abs(t[0] * R[2][2] - t[2] * R[0][2]) > ra + rb) return false;

		// Test axis L = A2 x B0
		ra = a.e[0] * AbsR[1][0] + a.e[1] * AbsR[0][0];
		rb = b.e[1] * AbsR[2][2] + b.e[2] * AbsR[2][1];
		if (Collision::Math::Abs(t[1] * R[0][0] - t[0] * R[1][0]) > ra + rb) return false;

		// Test axis L = A2 x B1
		ra = a.e[0] * AbsR[1][1] + a.e[1] * AbsR[0][1];
		rb = b.e[0] * AbsR[2][2] + b.e[2] * AbsR[2][0];
		if (Collision::Math::Abs(t[1] * R[0][1] - t[0] * R[1][1]) > ra + rb) return false;

		// Test axis L = A2 x B2
		ra = a.e[0] * AbsR[1][2] + a.e[1] * AbsR[0][2];
		rb = b.e[0] * AbsR[2][1] + b.e[1] * AbsR[2][0];
		if (Collision::Math::Abs(t[1] * R[0][2] - t[0] * R[1][2]) > ra + rb) return false;
